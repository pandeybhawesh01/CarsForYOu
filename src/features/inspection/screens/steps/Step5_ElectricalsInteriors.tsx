/**
 * Step 5 — Electricals + Interiors
 * Catalog-driven, same rules as Steps 1, 2, 3, and 4.
  ...
  if (input.inputType === 'multi-select') {
    ...
    return (
      <React.Fragment key={nodePath}>
        <MultiSelectChips key={nodePath} label={label} options={input.options} selected={current} onChange={(vals) => handlers.onMultiSelectChange(nodePath, vals)} />
      ...
      <React.Fragment key={nodePath}>
        ...
        {subOpts.map((sub, sIdx) => {
          ...
          if (subInputType === 'file-upload') {
            // Only shown when parent option (true) is selected — dispatch on sub.value (stable), not label
...
const Step5ElectricalsInteriors: React.FC<Props> = ({ onNext, onBack }) => {
  ...
  const handleNext = useCallback(() => {
    console.log('[Step5-Electricals] 🔄 Next button pressed');
    if (hasNextSection) {
      console.log('[Step5-Electricals] ➡️ Moving to next section:', mergedSections[currentSectionIndex + 1].key);
      setActiveTabKey(mergedSections[currentSectionIndex + 1].key);
      return;
    }
    if (canProceed) {
      console.log('[Step5-Electricals] ✅ Electricals & Interiors step complete - proceeding to next step');
      console.log('[Step5-Electricals] 📊 Current Electricals form data:', formData);
  ...
  if (loadingState === 'loading' && sectionNodes.length === 0) {
    return <SafeAreaView style={s.safeArea} edges={['bottom']}><AppHeader title="Electricals + Interiors" subtitle="Step 5 of 6" onBack={onBack} /><View style={s.centred}><ActivityIndicator size="large" color={colors.primary} /><Text style={s.loadingText}>Loading form…</Text></View></SafeAreaView>;
  }
  if (loadingState === 'error' && sectionNodes.length === 0) {
    return <SafeAreaView style={s.safeArea} edges={['bottom']}><AppHeader title="Electricals + Interiors" subtitle="Step 5 of 6" onBack={onBack} /><View style={s.centred}><Text style={s.errorIcon}>⚠️</Text><Text style={s.errorText}>Failed to load form fields.</Text><AppButton label="Retry" onPress={loadCatalog} size="sm" fullWidth={false} /></View></SafeAreaView>;
  }
    ...
    <SafeAreaView style={s.safeArea} edges={['bottom']}>
      ...
      <Modal visible={activeSlot !== null} animationType="slide" onRequestClose={handleCloseModal}>
        <SafeAreaView style={s.modalSafe} edges={['bottom']}>
          {activeSlot ? <InspectionImageDetailPanel title={activeSlot.label} issueOptions={activeSlot.issueOptions} value={photoDetails[activeSlot.storageKey]} onChange={handlePhotoChange} onBack={handleCloseModal} layout="photoFirstSubmit" listBackTitle="Electricals + Interiors" photoLabel="Photo" /> : null}
        </SafeAreaView>
      </Modal>
      ...
      <Modal visible={activeGroupNode !== null} animationType="slide" onRequestClose={handleCloseGroupModal}>
        <SafeAreaView style={s.modalSafe} edges={['bottom']}>
          {activeGroupNode ? (
            <>
              <AppHeader title={activeGroupNode.label} subtitle="Electricals + Interiors" onBack={handleCloseGroupModal} variant="primary" />
      ...
      <View style={s.footer}>
        {!hasNextSection && !canProceed && <Text style={s.hint}>⚠ Complete all required fields to proceed</Text>}
        <AppButton label="Next →" onPress={handleNext} isDisabled={!hasNextSection && !canProceed} testID="step5-next-btn" />
      </View>
    </SafeAreaView>
  );
};
