...
const Step3SteeringBrakes: React.FC<Props> = ({ onNext, onBack }) => {
  ...
  const formData = (currentSession?.formData.steeringBrakes ?? {}) as Record<string, unknown>;
  ...
  const handleTextChange = useCallback((path: string, value: string) => updateFormData(InspectionStepId.Interior, { [stripSectionPrefix(path)]: value }), [updateFormData]);
  const handleSelectChange = useCallback((path: string, value: string) => updateFormData(InspectionStepId.Interior, { [stripSectionPrefix(path)]: value }), [updateFormData]);
  const handleMultiSelectChange = useCallback((path: string, values: string[]) => updateFormData(InspectionStepId.Interior, { [stripSectionPrefix(path)]: values }), [updateFormData]);
  ...
  const handleNext = useCallback(() => {
    console.log('[Step3-Interior] 🔄 Next button pressed');
    if (hasNextSection) {
      console.log('[Step3-Interior] ➡️ Moving to next section:', mergedSections[currentSectionIndex + 1].key);
      setActiveTabKey(mergedSections[currentSectionIndex + 1].key);
      return;
    }
    if (canProceed) {
      console.log('[Step3-Interior] ✅ Interior step complete - proceeding to next step');
      console.log('[Step3-Interior] 📊 Current Interior form data:', formData);
  ...
  return (
    <SafeAreaView style={s.safeArea} edges={['bottom']}>
      ...
      <View style={s.footer}>
        ...
        <AppButton label="Next →" onPress={handleNext} isDisabled={!hasNextSection && !canProceed} testID="step3-next-btn" />
      </View>
    </SafeAreaView>
  );
};
